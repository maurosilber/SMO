...
n = 32  # image size
n2 = n // 2
window = 5  # averaging filter size

# True image
true_image = np.zeros((n, n))
true_image[:, n2:] = 10 * np.abs((np.arange(2, n2 + 2) % 4) - 2)
...
# Pipeline
pipeline = common.SMOPipeline(noisy_image, sigma=0, window=window)
...
# Image plots
for ax, title in zip(
    ax2d, ("Intensity image", "Gradient direction", "Averaged gradient", "SMO image")
):
...
# Gradient image
map_gradient = ax2d[1].quiver(
    *np.mgrid[:n:2, :n:2][::-1],
    *pipeline.gradient[:, ::2, ::2][::-1],
    pipeline.angle[::2, ::2],
    cmap=common.cmap.angle,
...
map_gradient = ax2d[2].quiver(
    *np.mgrid[:n:2, :n:2][::-1],
    *pipeline.averaged_gradient[:, ::2, ::2][::-1],
    pipeline.averaged_angle[::2, ::2],
    cmap=common.cmap.angle,
    norm=plt.Normalize(-np.pi, np.pi),
    scale_units="xy",
    scale=0.5,
...
common.save_or_show(fig, "suppfig1")
